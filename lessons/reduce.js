...
// Примечания
// Функцию headersCount можно использовать для наглядного сопоставления частного варианта
// свёртки с обобщённой реализацией операции отображения (собственно, reduce).
...
export const emptyTagsCount = (tag, elements) => {
  const func = function(element, acc) {
    if(is(tag, element) && getValue(element) === '') {
      acc = acc + 1;
    }
    return acc;
  }
