// Реализуйте и экспортируйте функцию map для библиотеки html-tags. Реализация должна быть
// построена с использованием итеративного процесса (без циклов, на основе рекурсии).
// Эта функция подобна той что описывалась в теории для списков, только текущая реализация
...
//   return element;
// }, dom3);
// Реализуйте и экспортируйте функцию mirror, которая переворачивает содержимое тегов, так
...
export const map = (func, elements) => {
  console.log(listToString(elements));
  if (isEmpty(elements)) {
    return l();
  }
...
export const mirror = (elements) => {
  let f = (element) =>{
    return node(name(element), reverseStr(value(element)));
  };
  return map(f, elements);
