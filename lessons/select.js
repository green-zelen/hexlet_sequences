// Работа с древовидными структурами в промышленном программировании достаточно частая
// ситуация. Например, вывод файловой структуры в нашем редакторе — типичный пример работы
// с деревьями.

// select.js
// Реализуйте и экспортируйте по умолчанию функцию select, которая принимает на вход имя
// тега и html список, а возвращает список всех нод, соответствующих имени. Ноды возвращаются
// в том виде, в котором они представлены в дереве. Порядок, в котором ноды возвращаются —
// не важен.
...
// Если текущая нода содержит детей, то запускаем select рекурсивно для детей, а результат
// вызова добавляем в аккумулятор.
...
import { /* eslint-disable */
  ...
  const select = (tagName, html) => {
    ...
    const f = (e, acc) => {
      // ver 3. RC
      if(is(tagName, e)) {
        return !hasChildren(e) ? concat(acc, l(e)) : concat(concat(acc, l(e)), select(tagName, children(e)))
      } else {
        return !hasChildren(e) ? acc : concat(acc, select(tagName, children(e)))
      }
  
      // ver 2
      if(is(tagName, e)) {
        if(!hasChildren(e)) {
          return concat(acc, l(e));
        } else {
          return concat(concat(acc, l(e)), select(tagName, children(e)))
        }
      } else {
        if(!hasChildren(e)) {
          return acc
        } else {
          return concat(acc, select(tagName, children(e)))
        }
      }
      ...
      if(!hasChildren(e)) {
        if(is(tagName, e)) {
          let r = concat(acc, l(e))
          console.log('OK acc = ' + listToString(r))
          return r
        } else {
          console.log('FAIL acc = ' + listToString(acc))
          return acc
        }
      } else {
        return concat(acc, select(tagName, children(e)))
